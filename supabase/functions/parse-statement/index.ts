...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${STATEMENT_MODEL}:generateContent?key=${GEMINI_API_KEY}`,
      {
        method: 'POST',
    ...
    // Normalization (strict is_transfer, account/FX field validation) lives in
    // parserConfig.ts so the offline golden test exercises the exact same code.
