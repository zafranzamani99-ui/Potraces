...
export type CollectzSocials = Partial<Record<CollectzSocialKey, string>>;
...
export interface CollectzSession {
  ...
  /** Organizer contact — canonical profile URLs keyed by platform (any subset). */
...
export function joinErrorMessage(code: string): string {
  // Services can't use the useT() hook — pick the language off the store directly.
  const t = useSettingsStore.getState().language === 'ms' ? ms : en;
  switch (code) {
    ...
    case 'session_closed': return t.collectz.joinSessionClosed;
...
export async function deleteSession(id: string): Promise<void> {
  ...
  // Club + QR images too (fixed per-session paths) — remove() tolerates missing files.
  await supabase.storage.from(IMAGES_BUCKET).remove([`${id}/club.jpg`, `${id}/qr.jpg`]).catch(() => {});
 ...
 * (weekly clubs); uploaded club + QR images are copied to the new session's
 * folder; share_code is freshly generated by createSession.
 */
export async function duplicateSession(sourceId: string): Promise<CollectzSession> {
  ...
  const created = await createSession({
    ...
    // qr_image_path is COPIED into the new folder below — never share the path.
    image_path: src.image_path,
    maps_url: src.maps_url,
    socials: src.socials,
    group_url: src.group_url,
    calc_notes: src.calc_notes,
  ...
  const copyImage = async (srcPath: string, field: 'image_path' | 'qr_image_path') => {
    const fileName = srcPath.split('/').pop() ?? 'club.jpg';
    const newPath = `${created.id}/${fileName}`;
    const { error: copyErr } = await supabase.storage.from(IMAGES_BUCKET).copy(srcPath, newPath);
    if (!copyErr) {
      await supabase.from('collectz_sessions').update({ [field]: newPath }).eq('id', created.id);
      created[field] = newPath;
    }
  };
  if (src.image_path && !src.image_path.startsWith('preset:')) {
    await copyImage(src.image_path, 'image_path');
  }
  ...
  for (const p of participants) {
    await addParticipant(created.id, p.name, {
      ...
      // them into an equal/flat session turns them into custom overrides that
      // stop shares summing to the total. Only true custom shares carry.
      share_amount: src.scheme === 'custom' ? p.share_amount : null,
      team_idx: p.team_idx,
    });
  }
  return created;
