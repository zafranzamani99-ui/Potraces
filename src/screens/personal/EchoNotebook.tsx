...
import { useLearningStore, TRUST_COUNT, normalizeKeyword } from '../../store/learningStore';
...
type RuleKind = 'category' | 'wallet' | 'person';
...
const EchoNotebook: React.FC = () => {
  ...
  }, [categoryPatterns, walletPreferences, personAliases]);
  ...
  const kindIcon = (kind: RuleKind): keyof typeof Feather.glyphMap =>
    kind === 'category' ? 'tag' : kind === 'wallet' ? 'credit-card' : 'user';

  const kindLabel = (kind: RuleKind) =>
    kind === 'category' ? t.echoNotebook.typeCategory : kind === 'wallet' ? t.echoNotebook.typeWallet : t.echoNotebook.typePerson;

  const ruleValueLabel = (r: NotebookRule) =>
    r.kind === 'category' ? categoryName(r.value) : r.value;
  ...
  const forgetRule = (kind: RuleKind, keyword: string, value: string) => {
    ...
    else s.forgetPersonAlias(keyword);
  };
  ...
  const handleResetAll = () => {
    ...
    Alert.alert(
      ...
      [
        { text: t.echoNotebook.cancel, style: 'cancel' },
        { text: t.echoNotebook.resetAll, style: 'destructive', onPress: () => useLearningStore.getState().resetNotebook() },
      ]
    );
  };
      ...
      >
        ...
        {total > 0 && (
          <View style={styles.filterRow}>
            {(['all', 'person', 'category', 'wallet'] as const).map((f) => {
...
const makeStyles = (C: ReturnType<typeof useCalm>) =>
  StyleSheet.create({
    ...
    filterRow: { flexDirection: 'row', gap: SPACING.sm, marginTop: SPACING.sm, marginBottom: SPACING.xs },
    pill: {
      flex: 1,
